...
func TestDefine(t *testing.T) {
	...
	a := global.Define("a")
	if a != expected["a"] {
		t.Errorf("expected a=%+v, got a=%+v", expected["a"], a)
	}

	b := global.Define("b")
	if b != expected["b"] {
		t.Errorf("expected b=%+v, got b=%+v", expected["b"], b)
	}
	...
	if c != expected["c"] {
		t.Errorf("expected c=%+v, got c=%+v", expected["c"], c)
	}

	d := firstLocal.Define("d")
	if d != expected["d"] {
		t.Errorf("expected d=%+v, got d=%+v", expected["d"], d)
	}
	...
	if e != expected["e"] {
		t.Errorf("expected e=%+v, got e=%+v", expected["e"], e)
	}

	f := secondLocal.Define("f")
	if f != expected["f"] {
		t.Errorf("expected f=%+v, got f=%+v", expected["f"], f)
	}
}
...
func TestResolveFree(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if len(tt.table.FreeSymbols) != len(tt.expectedFreeSymbols) {
			t.Errorf("Wrong number of free symbols. Got: %d. Expected: %d", len(tt.table.FreeSymbols), len(tt.expectedFreeSymbols))
			continue
		}
		...
		for i, sym := range tt.expectedFreeSymbols {
			...
			if result != sym {
				t.Errorf("Wrong free symbol. Got: %+v. Expected: %+v", result, sym)
			}
		}
	}
}
...
func TestResolveUnresolvableFree(t *testing.T) {
	...
	for _, name := range expectedUnresolvable {
		...
		if ok {
			t.Errorf("Name %s resolved, but was expected not to be", name)
		}
	}
}
